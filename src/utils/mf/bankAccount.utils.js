...
export const createFpBankAccount = async (payload) => {
  try {
    console.log("\n📤 [FP BANK] Create payload:", JSON.stringify(payload, null, 2));
    ...
    console.error("❌ [FP BANK] Create failed:", JSON.stringify(err.response?.data || err.message, null, 2));
    throw new Error(err.response?.data?.message || "Failed to create bank account");
  }
};
...
export const fetchFpBankAccount = async (bankAccountId) => {
  try {
    ...
    console.error("❌ [FP BANK] Fetch failed:", err.response?.data || err.message);
    throw new Error(err.response?.data?.message || "Failed to fetch bank account");
  }
};
