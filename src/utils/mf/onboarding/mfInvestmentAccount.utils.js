...
export const createFpMfInvestmentAccount = async (payload) => {
  try {
    ...
    console.log(
      "✅ [FP MF ACCOUNT] Response:",
      JSON.stringify(response.data, null, 2),
    );
    return response.data;
  } catch (err) {
    console.error(
      "❌ [FP MF ACCOUNT] Create failed:",
      JSON.stringify(err.response?.data || err.message, null, 2),
    );
    throw new Error(
      err.response?.data?.message || "Failed to create MF investment account",
    );
  }
};
...
export const updateFpMfInvestmentAccount = async (accountId, payload) => {
  try {
    ...
    console.log(
      "✅ [FP MF ACCOUNT] Updated:",
      JSON.stringify(response.data, null, 2),
    );
    return response.data;
  } catch (err) {
    console.error(
      "❌ [FP MF ACCOUNT] Update failed:",
      JSON.stringify(err.response?.data || err.message, null, 2),
    );
    throw new Error(
      err.response?.data?.message || "Failed to update MF investment account",
    );
  }
};
...
export const listFpMfInvestmentAccounts = async (params = {}) => {
  try {
    ...
    console.log("get list of existing user", response.data);
    return response.data;
  } catch (err) {
    console.error(
      "❌ [FP MF ACCOUNT] List failed:",
      err.response?.data || err.message,
    );
    throw new Error(
      err.response?.data?.message || "Failed to list MF investment accounts",
    );
  }
};
...
export const fetchFpMfInvestmentAccount = async (accountId) => {
  try {
    ...
    console.error(
      "❌ [FP MF ACCOUNT] Fetch failed:",
      err.response?.data || err.message,
    );
    throw new Error(
      err.response?.data?.message || "Failed to fetch MF investment account",
    );
  }
};
