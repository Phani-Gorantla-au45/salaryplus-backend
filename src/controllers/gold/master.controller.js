...
import { AugmontState, AugmontCity } from "../../models/gold/state.model.js";
...
export const syncStates = async (req, res) => {
  try {
    ...
    const bulkOps = states.map((s) => ({
      updateOne: {
        filter: { stateId: s.id },
        update: { stateId: s.id, name: s.name },
        upsert: true,
      },
    }));

    await AugmontState.bulkWrite(bulkOps);
...
export const getStatesFromDB = async (req, res) => {
  try {
    const states = await AugmontState.find().sort({ name: 1 });
    res.json(states);
  } catch (err) {
...
export const syncCities = async (req, res) => {
  try {
    ...
    await AugmontCity.bulkWrite(bulkOps);
