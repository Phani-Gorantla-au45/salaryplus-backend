...
export const calculateGoalPreview = async (req, res) => {
  try {
    ...
    if (!templateId || !inputs)
      return res.status(400).json({ message: "templateId and inputs are required" });
...
export const saveUserGoal = async (req, res) => {
  try {
    ...
    const { targetAmount, monthlySip, duration } = calculateGoal(
      template.type, inputs, template.assumptions
    );
...
export const deleteUserGoal = async (req, res) => {
  try {
    const { uniqueId } = req.user;
    const deleted = await UserGoal.findOneAndDelete({ _id: req.params.id, uniqueId });
