...
//   }
// };
// controllers/metalIntent.controller.js
import axios from "axios";
import generateJWS from "../juspay/generateJWS.js";
...
export const createMetalIntent = async (req, res) => {
  try {
    ...
    /* ------------ INPUT ------------ */
    const { metalType, amount, quantity, lockPrice, blockId } = req.body;
    ...
    /* ------------ AMOUNT / QUANTITY RULE ------------ */
    if ((amount && quantity) || (!amount && !quantity)) {
      return res.status(400).json({
        message: "Pass either amount or quantity",
      });
    }
    ...
    if (!lockPrice || !blockId) {
      return res.status(400).json({
        message: "lockPrice & blockId required",
      });
    }
    ...
    const payload = {
      merchantRequestId: merchantTransactionId,
      amount: formattedAmount, // Juspay ALWAYS needs amount
      currency: "INR",
      flow: "TRANSACTION",
      remarks: "Metal purchase",
      intentRequestExpiryMinutes: "3",
